...
function ldap_find_user_info ($login)
{
	...
	//echo "Connecting ...";
	//	echo "<p>$login for $ldap_host : $ldap_port";
	...
	if ($ldap_connect) {
		...
		if ($ldap_bind_res)
		{
			...
			//	echo "<p>Base = '$LDAPbasedn' ou Search = '$ldap_search_dn' </p>";
			$ldap_search_dn = '';
...
//	print_r($info);
	$result["username"] = $info[0]["cn"][0];
	$result["firstname"] = $info[0]["givenname"][0];
	$result["lastname"] = $info[0]["sn"][0];
...
//	$result[$tutor_field] = $info[0][$tutor_field]; //employeenumber by default
  if (isset($info[0]["dn"][0])) {
            $result['studentId'] = $info[0]["dn"][0];
        } 
	return $result;
}
/**
...
function ldap_put_user_info_locally($login, $info_array)
{
	...
	$nom        = api_convert_encoding($info_array["name"], api_get_system_encoding(), 'UTF-8'); //$info_array["name"];
	$prenom     = api_convert_encoding($info_array["firstname"], api_get_system_encoding(), 'UTF-8');//$info_array["firstname"];
...
{
	//	echo 'Entering ldap_authentication_check('.$uname.','.$passwd.')';
...
//	define(LDAP_OPT_DIAGNOSTIC_MESSAGE, 0x0032);
	...
	//	echo '<br>connect to: '.$ldap_host.':'.$ldap_port.'<br />';
	$ds=ldap_connect($ldap_host,$ldap_port);
	if ($ds)
	{ 
//		echo '<br>$ds: LDAP link identifier: '.$ds;
...
//			echo '<br>Searching for '.$filter.' on LDAP server';
	    ...
	    if ($ds) {
			...
			if (@ldap_bind($ds, $dn , $passwd) === false) {
				...
				if (ldap_get_option($ds, LDAP_OPT_DIAGNOSTIC_MESSAGE, $extended_error)) {
					echo "$ds, $dn , $passwd";
     				echo "<br>Error Binding to LDAP: $extended_error";
    			} else {
...
//			echo "Could not connect 2 to $ldap_host";
			return(-1);
		}
	}else{
		echo ldap_error($ds) . "<br/>";
//		echo "Could not connect 1 to $ldap_host";
		return(-1);
	}
} // end of check
/**
...
{
//	echo ('Entering ldap_handle_bind(&$ldap_handler,&$ldap_bind)');
	global $ldap_rdn,$ldap_pass;
	if(!empty($ldap_rdn) and !empty($ldap_pass))
	{
//		echo ('<br>Trying authenticated login :'.$ldap_rdn.'/'.$ldap_pass);
		$ldap_bind = ldap_bind($ldap_handler,$ldap_rdn,$ldap_pass);
		if(!$ldap_bind)
		{
//			echo ('<br>Authenticated login failed');
